...
#ifndef SYNAPSE_RECORDER_H
#define SYNAPSE_RECORDER_H
...
void register_synapse_recorder( const std::string& name );

class synapse_recorder : public RecordingDevice
{

public:
  synapse_recorder();
  synapse_recorder( const synapse_recorder& );
  ...
  void handle( SynapseRecorderEvent& ) override;

  size_t handles_test_event( SynapseRecorderEvent&, size_t ) override;
...
inline size_t
synapse_recorder::handles_test_event( SynapseRecorderEvent&, size_t receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline SignalType
synapse_recorder::receives_signal() const
{
  return ALL;
}
...
#endif /* #ifndef SYNAPSE_RECORDER_H */
