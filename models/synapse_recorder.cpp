...
void
nest::register_synapse_recorder( const std::string& name )
{
  register_node_model< synapse_recorder >( name );
}

// record time, node ID, weight and receiver node ID
nest::synapse_recorder::synapse_recorder()
  : RecordingDevice()
  , P_()
{
}

nest::synapse_recorder::synapse_recorder( const synapse_recorder& n )
  : RecordingDevice( n )
  , P_( n.P_ )
{
}
...
nest::synapse_recorder::Parameters_::Parameters_()
...
void
nest::synapse_recorder::Parameters_::get( Dictionary& d ) const
{
...
void
nest::synapse_recorder::Parameters_::set( const Dictionary& d )
{
...
void
nest::synapse_recorder::pre_run_hook()
{
...
void
nest::synapse_recorder::update( Time const&, const long, const long )
{
}

nest::RecordingDevice::Type
nest::synapse_recorder::get_type() const
{
  return RecordingDevice::SYNAPSE_RECORDER;
}

void
nest::synapse_recorder::get_status( Dictionary& d ) const
{
...
void
nest::synapse_recorder::set_status( const Dictionary& d )
{
  Parameters_ ptmp = P_;
  ptmp.set( d );
...
void
nest::synapse_recorder::handle( SynapseRecorderEvent& e )
{
